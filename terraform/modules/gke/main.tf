data "google_container_engine_versions" "default" {
  ...
  location       = var.region
  version_prefix = "1.27.6-gke.1506000"
}
resource "google_container_cluster" "primary" {
  provider = google

  name            = var.name
  description     = var.description // need to add
  project         = var.project_id
  resource_labels = var.cluster_resource_labels // need to add
  ...
  min_master_version = var.release_channel == null || var.release_channel == "UNSPECIFIED" ? var.master_version : var.kubernetes_version == "latest" ? null : var.kubernetes_version
  # node_version  = data.google_container_engine_versions.stable
  ...
  dynamic "monitoring_config" {
    for_each = length(var.logging_enabled_components) > 0 || length(var.monitoring_enabled_components) > 0 ? [1] : []
    content {
  ...
  #   }
  #   resource_limits {
  #     resource_type = "cpu"  ////dynamic pending
  #     minimum = 1
  #     maximum = 2
  #   }
  #   resource_limits {
  ...
  enable_kubernetes_alpha = var.enable_kubernetes_alpha
   ...
   master_authorized_networks_config {
     cidr_blocks {
       cidr_block = var.private_ipv4_cidr
       display_name = "private-ipv4-cidr"
     }
   }
 ...
 resource "google_container_node_pool" "general" {
  ...
  dynamic "autoscaling" {
    ...
    content {
      ...
      total_min_node_count = lookup(autoscaling.value, "min_count", null)
      total_max_node_count = lookup(autoscaling.value, "max_count", null)
    }
  }
  node_config {
    preemptible  = false
